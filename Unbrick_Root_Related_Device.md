...
关键字：变砖、救砖、Root、KernelSU、init_boot、boot、vbmeta

在用户提出上述关键字相关问题和求助时，先询问明确用户之前做了什么，然后按照下方分支判定：
...
2. 用户指出刷写镜像后就开不了机
   ...
   - 如果用户刷写的是GKI，对应的是**boot**镜像，按照下面的步骤操作：
     
     1. 询问对方刷写的是自行签名/自己从网上找到的镜像，还是群内的镜像
        
        - 如果对方使用自行签名的镜像，要求对方换用群内预先签名的镜像，并指出很可能是avb签名不正确
     ...
     2. 确认对方是否刷入到的是**boot**分区而非**init_boot**分区：
        ...
        - 如果对方刷入到**boot**分区，建议寻找其他群员求助
        
        - 如果对方刷入到**init_boot**或者**vbmeta**分区，**必须立刻**要求对方寻求售后协助，因为此时底层分区已经被破坏
